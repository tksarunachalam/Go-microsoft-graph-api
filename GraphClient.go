...
// grabbed. If grabbing a MsToken fails the JSON-Unmarshal returns an error.
type GraphClient struct {
	...
	MsToken Token // the current MsToken to be used
}
...
func (g *GraphClient) String() string {
	...
	return fmt.Sprintf("GraphClient(TenantID: %v, ApplicationID: %v, ClientSecret: %v...%v, Token validity: [%v - %v])",
		g.TenantID, g.ApplicationID, firstPart, lastPart, g.MsToken.Ext_expires_in, g.MsToken.ExpiresIn)
}

// NewGraphClient creates a new GraphClient instance with the given parameters and grab's a MsToken.
//
// Rerturns an error if the MsToken can not be initialized. This method does not have to be used to create a new GraphClient
func NewGraphClient(tenantID, applicationID, clientSecret string) (*GraphClient, error) {
	...
	g.apiCall.Lock()         // lock because we will refresh the MsToken
	defer g.apiCall.Unlock() // unlock after MsToken refresh
...
func (g *GraphClient) refreshToken() error {
	...
	resource := fmt.Sprintf("/%v/oauth2/v2.0/token", g.TenantID)
	data := url.Values{}
...
func (g *GraphClient) makeGETAPICall(apicall string, getParams url.Values, v interface{}) error {
	...
	// Check MsToken
	if g.MsToken.WantsToBeRefreshed() { // Token not valid anymore?
		err := g.refreshToken()
		if err != nil {
			return err
		}
	...
	req.Header.Add("Authorization", g.MsToken.GetAccessToken())
...
func (g *GraphClient) GetMyProfile() (Me, error) {
	resource := "/me"
	me := Me{graphClient: g}
	err := g.makeGETAPICall(resource, nil, &me)
...
func (g *GraphClient) GetJoinedTeams() (TeamResponse, error) {

	resource := "/me" + "/" + "joinedTeams"
	allTeams := TeamResponse{graphClient: g}

	err := g.makeGETAPICall(resource, nil, &allTeams)
...
// and returns an error if any of the data provided is incorrect or the MsToken can not be acquired
func (g *GraphClient) UnmarshalJSON(data []byte) error {
	...
	// get a MsToken and return the error (if any)
	err = g.refreshToken()
	if err != nil {
